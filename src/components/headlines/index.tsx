...
import { getHeadlines } from "../../plugins/store/content/actions";
...
const Headlines = () => {
    ...
    const headlines = useSelector((store: Store) => store.content.headlines) as Array<Content>;
    ...
    const moveSliderPosition = (position: number) => {
        ...
        setHeadlinesPosition(position < 0 ? headlines.length - 1 : position > headlines.length - 1 ? 0 : position);
    };
            ...
            <div className="w-full bg-red-900 rounded-xl img-on-center overflow-hidden flex flex-wrap items-center" style={{ height: "550px", zIndex: 998 }}>
                ...
                {headlines.length > 0 ? (
                    <div
                        ...
                        style={{
                            backgroundImage: "linear-gradient(to bottom, transparent 0%, black 120%), url(" + API_BASE_URL + "images/" + headlines[headlinesPosition].thumb + ")",
                            width: "100%",
                            height: "550px",
